...
const char * const BITCOIN_CONF_FILENAME = "fxrate.conf";
const char * const BITCOIN_PID_FILENAME = "fxrated.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "fxrate" is a composite category enabling all FXRate-related debug output
            if(ptrCategory->count(string("fxrate"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\FXRateCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\FXRateCore
    // Mac: ~/Library/Application Support/FXRateCore
    // Unix: ~/.fxratecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "FXRateCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/FXRateCore";
#else
    // Unix
    return pathRet / ".fxratecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty fxrate.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override fxrate.conf
